...
function Project() {
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data } = await Axios.get("/orders/15", {
        headers: {
  ...
  return (
    <div className="container preview">
      <div className="row">
        ...
        <p className="justify-content-start  mt-5">اسم المشروع  / {post?post.name:""}</p>
        <p className="justify-content-start  mt-5">نوع النشاط  / {post?post.acitivity:""}</p>
        <p className="justify-content-start  mt-5">المساحة المطلوبة  / {post?post.requiredArea:""}</p>
          <p className="justify-content-start  mt-5">
        ...
        <p className="justify-content-start  mt-5">حجم العمالة  / {post?post.employment:""}</p>
        <p className="justify-content-start  mt-5"> الشكل القانوني / {post?post.legalStatus:""}</p>
          <p className="justify-content-start  mt-5">مده التنفيذ / {post?post.duration:""}</p>
          <p className="justify-content-start  mt-5">
            كميه الإنتاج السنوي المتوقع
        / {post?post.expectedAnnualProductionAmount:""}</p>
          <p className="justify-content-start  mt-5">
        ...
        <p className=" mr-auto  mt-5 col-6">
          مقدمة لسيادتكم
          <br />
          الاسم: {post?post.customer.username?post.customer.username:"":""}
        </p>
